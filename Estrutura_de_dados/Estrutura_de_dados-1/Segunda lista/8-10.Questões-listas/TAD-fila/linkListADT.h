...
void _delete(LIST *pList, NODE *pPre, NODE *pLoc, void **dataOutPtr) {
  ...
  *dataOutPtr = pLoc->dataPtr;
  if (pPre == NULL) {
    pList->head = pLoc->link;   // Deleting first node
  } else {
    pPre->link = pLoc->link;    // Deleting any other node
  }

  if (pLoc->link == NULL) {
    pList->rear = pPre;         // Test for deleting last node
  }

  (pList->count)--;
  free(pLoc);

  // return; //necessario? acho que nao
} // _delete
...
#define COMPARE_LAST  ((*pList->compare)(pArgu, pList->rear->dataPtr))
  ...
  *pLoc = pList->head;
  ...
  if (result == 0) {
    return true;    // argument found--success
  } else {
    return false;
  }
} // _search
